...
class ConnectionManager:
    ...
    def disconnect(self, websocket: WebSocket):
        """Откючение пользователя."""
        self.active_connections.remove(websocket)

    async def send_personal_message(self, message: str, websocket: WebSocket):
        """Отправка сообщения опредленному пользователю."""
...
async def websocket_endpoint(websocket: WebSocket, client_id: int):
    ...
    try:
        while True:
            ...
            await manager.broadcast(f"Клиент #{client_id} написал: {data}", add_to_db=True)
    except WebSocketDisconnect:
        manager.disconnect(websocket)
        await manager.broadcast(f"Клиент #{client_id} покинул чат.", add_to_db=False)
